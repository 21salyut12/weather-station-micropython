import gc
from controller.config_lora import config_lora
...
class SX127x:
    ...
    def init(self, parameters=None):
        ...
        if (
            1000
            / (
                self.parameters["signal_bandwidth"]
                / 2 ** self.parameters["spreading_factor"]
            )
            > 16
        ):
            self.writeRegister(
    ...
    def aquire_lock(self, lock=False):
        if (
            not config_lora.IS_MICROPYTHON
    ...
    def read_payload(self):
        ...
        # read packet length
        packetLength = (
            self.readRegister(REG_PAYLOAD_LENGTH)
            if self._implicitHeaderMode
            else self.readRegister(REG_RX_NB_BYTES)
        )
